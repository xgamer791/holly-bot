// Holly Computer itself (computer/holly-computer.mjs), which this app runs on
// the Node.js it comes with, with the options from its settings
// (settings.js), as `node holly-computer.mjs` runs elsewhere.
//
// The file is kept current the way Holly Computer keeps itself current
// (computer/src/update.mjs): as it starts, the Holly Bot site is asked for the
// latest one, and while it runs, Holly Computer says when a newer one is out
// and no bot is working; that one is fetched, and Holly Computer restarts to
...
//
// What Holly Computer prints goes to the Activity tab and a log file. How
...
/** Lines of what Holly Computer printed that the Activity tab keeps. */
const MAX_LINES = 2000;
/** The log file starts again past this size (the last one is kept as .old). */
const LOG_LIMIT = 2 * 1024 * 1024;
/** How long Holly Computer gets to stop by itself (telling the account it's stopping) before it's made to. */
const STOP_MS = 15_000;
/** Holly Computer that stops by itself after running this long is started again; sooner, it failed to start. */
...
export class HollyComputer extends EventEmitter {
  ...
  constructor({ resources, home }) {
    ...
    this.status = 'stopped';
    /** How things stand there, as Holly Computer last said (computer/src/main.mjs tellState). */
    this.state = null;
  ...
  /** Starts Holly Computer with `settings` (settings.js); `newToken`: with new keys (--new-token). */
  ...
  fail(err) {
    this.line(`  Holly Computer couldn't start: ${err?.message || err}`);
    this.error = { kind: 'crash', detail: err?.message || String(err) };
    this.set('failed');
  }

  /** Stops Holly Computer: it tells the account it's stopping, and after
  ...
  onExit(child, code, signal) {
    ...
    if (now - this.startedAt >= STEADY_MS && this.restarts.length < MAX_RESTARTS) {
      ...
      this.line(`  Holly Computer stopped (${signal || `code ${code}`}). Starting it again…`);
      this.set('restarting');
      const run = this.run;
      setTimeout(() => {
  ...
  /**
   * Holly Computer says a newer one is out and no bot is working: it's
   * fetched, and then Holly Computer restarts to run it as soon as no bot is
  ...
  async prepare(update) {
    ...
    if (latest && (!current || newerVersion(latest.version, current)) && await this.save(latest.text)) {
      this.line(current ? `  Updated Holly Computer from ${current} to ${latest.version}.` : `  Got Holly Computer ${latest.version}.`);
      script = this.own;
    }
    return script;
  }
  ...
  async save(text) {
    ...
    try {
      ...
      this.line(`  Couldn't update Holly Computer (${err.message}).`);
      return false;
    }
  }

  /**
   * What Holly Computer runs with: this app's environment without Electron's
   * own variables (with those, an Electron app a bot starts from the shell
   * would run as Node), and HOLLY_DESKTOP, which opens its channel to this
   * app. The Node.js this app comes with goes on the end of the PATH, so bots
   * have node, npm and npx (and plugins started with npx work) whether or not
   * Node.js is installed; one that is comes first.
   */
  env() {
    const env = {};
  ...
  /** One line of what Holly Computer printed (or of what this app says about it). */
  line(text) {
