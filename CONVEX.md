...
The app and Holli Bot Computer talk to production (`CONVEX_URL` in `src/account/config.js`).

Do not point Forge (`polished-ram-883` / `zealous-partridge-60`) or Macronaut at Holli Bot, or Holli Bot at them. The deploy workflow refuses any key that isn't for `impressive-ferret-800`.
...
- `convex/devices.ts`: linking Holli Bot Computer to an account (below).
...
- `convex/ai.ts` and `convex/credits.ts`: Holli Bot's AI, DeepSeek on Holli Bot's key at `https://impressive-ferret-800.convex.site/ai/chat/completions`, and each account's AI credits for it, monthly or on Free daily, with Free's limits and daily budget (below), with `convex/lib/credits.ts` (DeepSeek's prices, what a request costs, the months and days of credits).
...
- **Isolation.** Every function in `convex/data.ts` gets the account from the verified session (`requireUserId` in `convex/lib/auth.ts`; the session must still exist, so signing out or deleting the account cuts off its tokens at once) and reads and writes only through indexes that start with that account.
- **Every account.** The same functions work for every signed-in account, on Free or a paid plan (since 1.43.0; before, only while its subscription was active). The client never names an account. Uploads are claimed by the account whose record first refers to them (`blobs`), and only that account can get a download URL.
- **Free's files.** Each upload's size is kept on its `blobs` row as it's claimed, and `heads.bytes` adds them up (files from before 1.43.0 aren't counted). A Free account keeps at most 100 MB (`FREE.storage` in `convex/lib/plans.ts`): `data:apply` refuses a record whose files would go over, and `data:uploadUrl` refuses once it's full, with "Free keeps up to 100 MB of files in your account". The app shows that as a toast and sets the change aside. Deleting a record gives its files' space back.
...
## Holli Bot Computer on the account

A linked Holli Bot Computer keeps its bots in the account with the same storage the app uses, and runs them (`computer/src/home.mjs`). Until it's linked it keeps them in its data folder (`computer/src/node-db.mjs`).
...
- **Unlinking.** The Bot Computer settings (the computer button at the top right, its last tab): from the computer itself (it ends its session and forgets the account's files) or from any signed-in device (`devices:unlink` ends the computer's session; the computer notices within a minute). Deleting the account ends it too. The bots stay in the account either way. The server that comes with a plan can't be unlinked: `devices:unlink` refuses it, and so does Holli Bot Computer on it, which `devices:report` tells whether it's the plan's server.
- **Linking by signing in.** Signing in on Holli Bot Computer's own page (localhost) links it straight away, with no button to tap (`computerAccountStep` in `src/main.js`): the person is at the computer. That page never offers Connect: until a phone has connected, it says to tap Connect on the phone.
...
- **Finding it.** While it runs, a linked computer tells the account where the account's devices can reach it (`devices:report`, from its own session and only for itself): its public https address (its tunnel, on by default, or `--public-url`) and an access key of its own, as it starts, whenever that changes, every five minutes, and as it stops (`stopping`, also when its window closes). A quick tunnel's address is told only once `cloudflared` has connected and the address answers `/v1/health` as this run of Holli Bot Computer (`instance`); it's checked every half minute, and when it stops answering the computer reports no address and opens a new tunnel (`TunnelKeeper` in `computer/src/tunnel.mjs`). With no address, `tunnel` says why: `off`, `starting` or `blocked` (its network blocks Cloudflare Tunnel). It also says its system (`platform`: `win32`, `darwin` or `linux`), and the answer says which of the account's computers it is (`id`): with `devices:list`, its bots know which of the user's computers they're working on and what the others are doing, by name, never an address or key (`computer/src/home.mjs` refreshComputers, `src/core/prompts.js` Your computers). `devices:list` hands them to the account's signed-in devices, and the app connects with them by itself when it opens (`openComputer` in `src/main.js`) to paired computers, and again when the computer stops answering because it restarted at a new address. While the app is open it asks `devices:list` every ten seconds (in front only), checks that a computer that came on answers at its address (`probeComputer`), and then, while the app runs the bots itself, connects to it by itself if it's paired, or offers Connect if it isn't (`watchComputers`); while it's on a computer, only one that isn't paired is offered. Each bot goes back to the computer it last used, kept on the device (`botComputer` in `src/remote/remote-app.js`): opening its chat switches the app there once it answers. Otherwise the app opens on, and goes back to, the computer the person was last using (`lastComputer`), the plan's server included. Not now is kept on the device for that address (twelve hours at most). When the computer the app uses hasn't answered for two minutes, it moves to another paired one that does. A computer not heard from for twelve minutes counts as off. The access key isn't the pairing token in the QR code: the computer makes it when it's linked (kept in `<data>/account.json`), accepts it as well as the pairing token, and forgets it when it's unlinked, so a device that got it from the account can't reach the computer after that. `--new-token` changes both.
...
- **Higgsfield, with nothing to set up.** Its MCP server has a sign-in of its own (`https://mcp.higgsfield.ai/oauth2`, in front of Clerk's at `clerk.higgsfield.ai`) that lets apps register themselves (RFC 7591), so `connectors:start` registers a public client there for each connection, with its callback, and the person signs in to Higgsfield with PKCE; the tokens are for that server (`https://mcp.higgsfield.ai/mcp`, the `resource`) and carry the client they were issued to, for renewing and revoking. (1.30.0 registered with Clerk directly, and its connections couldn't reach the tools.) It works as soon as `CONNECTORS_KEY` exists. `connectors:run` then lists Higgsfield's tools (`tools`) and runs one (`call`), each in an MCP session of its own (`convex/lib/higgsfield.ts`): that server turns browsers away, so the app can't call it itself. Generations use the person's own Higgsfield credits.
...
Every account starts on Free (`FREE` in `convex/lib/plans.ts`). A new account (on Free, with no bots) sees the plan page first and picks a plan or Continue with Free before its first bot, the Chief Coordinator (`showPlans` in `src/main.js`); an account whose paid plan ends goes back to Free. Upgrade Plan at the top of Settings opens the plan page over the app (`PlansPage` in `src/ui/subscribe.js`). Back from paying, the app waits on that page for Stripe's word (`subscribed()` in `src/main.js`), then opens on the new plan without waiting for the computer: an account with no bots makes its first one, the Chief Coordinator (`src/ui/chief.js`), the bots run in the app meanwhile with the computer button pulsing blue, and the app moves onto the computer once it's ready (`watchServerSetup` in `src/main.js`). Free isn't a Stripe plan: it isn't in `PLANS`, has no price and never gets a server, and what it may do (its credits, Flash only, its files) is enforced on the server, so a copy of the app with the checks taken out gets nowhere.
...
- **Plans** live in `convex/lib/plans.ts`: names, prices, server sizes, the region (`ord`, Chicago) and the operating system (Ubuntu 24.04), and Free's limits (`FREE`). The page shows them as `billing:status` hands them out, with `free` (the account is on Free) and `freeCredits`.
...
- **The webhook** (`/stripe/webhook`) checks every event's signature (`STRIPE_WEBHOOK_SECRET`) and refuses anything else, and handles each event once: its id goes in `stripeEvents` in the same transaction as the change, and a repeat is skipped. Each event reads the subscription it's about from Stripe again, so events that arrive late or out of order can't undo a newer change.
  ...
  - `invoice.payment_failed`: past due. Holli Bot keeps working and the server keeps running, and the app shows a "Your payment didn't go through" banner that opens the billing portal.
  - `customer.subscription.deleted`: canceled. The server is deleted, and the app reloads on Free.
- **Who's on a paid plan:** Stripe's status `active` or `trialing`, or `past_due` while Stripe tries the card again. Anything else (`unpaid`, `paused`, `canceled`…) is on Free; for a subscription that isn't over (`unpaid`, `paused`, `incomplete`), the plan page leads to the billing portal rather than to a second subscription, and `billing:checkout` refuses one while the account has such a subscription. `unpaid` also gets the payment banner.
- **Exempt accounts** (`EXEMPT` in `convex/lib/subscription.ts`): an account listed there gets the biggest plan's AI credits without a subscription (the owner's, while they test Holli Bot). It's listed by the SHA-256 of its verified email address, so the address isn't in the code (`printf %s you@example.com | sha256sum`). It isn't on Free, but gets no server; delete or comment out its line to put it on Free.
...
- **Managing.** Stripe's billing portal (`billing:portal`): change plan, update the card, see invoices, cancel. The app opens it from Settings → Plan (the plan page's Manage Billing) and from the banner about a payment that didn't go through. `servers:retry` sets a computer up again after it failed.
- **What Free can do:** everything but a server: `data:*` (with at most 100 MB of files), Gmail, Outlook, GitHub and Higgsfield, linking computers of its own, and Holli Bot's AI within Free's limits (below).
- **Test and live.** A subscriber's row remembers which Stripe mode made it, and only the mode of the current `STRIPE_SECRET_KEY` counts, so a test subscription never opens Holli Bot once the key is live (and the nightly reconcile then deletes its server).
...
- The AI is included: each plan comes with monthly AI credits for Holli Bot's AI, and Free with daily ones (below).

## Holli Bot's AI and AI credits

Bots think with DeepSeek on Holli Bot's own key, `DEEPSEEK_API_KEY`, which only the deployment holds. Each plan gives the account AI credits every month (`credits` in `convex/lib/plans.ts`: Starter $10, Pro $20, Ultra $35 of DeepSeek use at DeepSeek's list prices, which the app shows as 1,000, 2,000 and 3,500 credits), and Free every day (`FREE.credits`: 10, 10¢). There are no API keys for AI in the app any more.

- **Requests.** The app and Holli Bot Computer send each bot request, OpenAI-style, to `POST /ai/chat/completions` (`convex/ai.ts`) with the account's session JWT as the bearer token (`sessionToken` on `src/account/cloud-db.js`, used by `src/core/providers/index.js`). The route checks the session is still open, takes only DeepSeek's chat parameters and the models `deepseek-flash` and `deepseek-v4-pro` (and `glm-flash`, Flash's name in 1.25.0, which runs as `deepseek-flash`), caps the output at 65,536 tokens, lets it in if the account has credits (`credits:admit`, which also applies Free's limits) and passes it to `https://api.deepseek.com/chat/completions`. Streamed answers go back to the app as DeepSeek sends them. Nothing of a request or answer is kept.
...
- **Holds.** Before a request goes to DeepSeek, `credits:admit` holds back the most it could cost (all its input new, all the output it may ask for), or what isn't held for other requests already when that's less, as a row of its own in `creditHolds`, and `charge` lets it go as it charges. So requests at once can't spend more than is left, and an account never goes more than one request's worth below zero. Holds are kept apart from the credits, so what's left, as `credits:mine` gives it to the app's bar, only ever goes down by what requests cost, never by a hold for the moment a request runs. When everything left is held for requests under way, a new one gets `credits_busy` ("Your bots are using the rest of your AI credits right now") rather than "used up". A hold whose request never came back to be charged (the request was cut off with no word) is charged in full after an hour, the most it could have cost, at the account's next request or by the hourly sweep (`convex/crons.ts`). The app asks `credits:mine` with the minute (`minute`), since Convex keeps a query's answer until what it read changes, and a refill at midnight isn't a change.
- **Months, or days on Free.** An account's credits (`credits` table) count months from the day its paid period renews, so a monthly plan's refill as it's billed, and a yearly plan's refill monthly on that day. On Free (`daily`), each UTC day is a period of its own. A new period sets the balance to the allowance; nothing carries over. A plan changed during the month adds the difference (bigger) or caps what's left (smaller); moving between Free and a paid plan starts a new period at once, full. All of it happens as the account's credits are next read or used, with no cron.
- **At zero,** `/ai/chat/completions` answers 402 with `code: "no_credits"` and when they refill (on Free, `free_credits`, with the hours to midnight UTC); the app shows that on the bot's reply, with a See credits button (and on Free, Upgrade Plan), and the bot pauses until then. Exempt accounts get the biggest plan's credits.
- **Free's limits** (`credits:admit`, which answers each with a code the app shows on the reply with Upgrade Plan): only `deepseek-flash` (`free_model`, 403; the app runs a Free account's bots on Flash anyway); at most 16,384 tokens of output (a request asking for more gets that) and 131,072 of input as `promptTokens` counts it (`free_too_long`, 413; the app keeps a Free bot's chat history to 64,000 tokens); and the daily budget below (`free_capacity`, 503). Peak hours cost Free double like everyone, since credits are charged at DeepSeek's rates.
- **Free's daily budget.** All Free accounts together spend at most `FREE_DAILY_BUDGET_USD` a day, in US dollars (`5` when it isn't set; `0` turns Free's AI off). `freeSpend` keeps one row per UTC day: each Free request's hold is added as it's let in, and replaced by what it cost once it's charged. Once the day's row reaches the budget, Free requests get `free_capacity` ("Free is at capacity today") until midnight UTC; paid and exempt accounts are never affected. A new day is a new row, so nothing resets. Set it with `npx convex env set FREE_DAILY_BUDGET_USD 5` or in the dashboard.
- **The app** reads `credits:mine` for Settings → Usage: a bar of what's left, the credits left and the refill date (on Free, the time today), and `plan` (`free`, `paid` or `exempt`). It also says `ready` (whether `DEEPSEEK_API_KEY` is set); until it is, bots keep using a DeepSeek key an account had saved before credits, and accounts without one can't run bots.
- **DeepSeek's balance** pays for everyone. When DeepSeek turns Holli Bot's key down (out of balance, or a wrong key), members see "Holli Bot's AI is unavailable right now", and the deployment's logs say `DeepSeek refused Holli Bot's key` with the reason. Keep the DeepSeek account topped up: at most, a month costs the sum of every subscriber's allowance, plus Free's daily budget every day.

## The Bot Store

Pre-trained bots, bought once (`convex/store.ts`, `convex/lib/store.ts`; the app's side is `src/ui/store.js`).

- **The catalog** is `storeBots`: each bot's name, tagline, about (its page in the store, in Markdown, up to 8,000 words), category, look, price (US cents, $10 at least), what it can do, its pre-trained memory (up to 10,000 words), the rules it comes with, and whether it's listed and featured. The owner adds and changes bots in the app (Settings → Bot Store → Manage: `store:manage`, `store:save`), as the account whose email's SHA-256 is in `ADMINS` (`convex/lib/store.ts`), the way EXEMPT works. Until a bot is listed, the sample bots in `convex/lib/storeSamples.ts` are on sale instead.
- **Buying:** `store:catalog` lists what's on sale and what the account bought; `store:checkout` makes a Stripe Checkout session (mode=payment, cards with Apple Pay and Google Pay, the account's Stripe customer, the price from the catalog) with `metadata.kind = "store"`, and Stripe sends the person back with `?store=done&bot=…&session=…`. `store:claim` asks Stripe for that session and keeps the purchase if it's this account's and paid; the webhook keeps it too, on `checkout.session.completed` (no new event to subscribe to). Purchases are `storePurchases`, one per Checkout session, counted in their own Stripe mode like subscriptions; they're deleted with the account (Stripe keeps the payments). A refund in Stripe doesn't take the bot back: to take its memory back too, delete the purchase's row in `storePurchases`.
- **Adding the bot:** `store:install` hands the app a bought bot's name, look, tagline and rules (never its memory) for the accounts that bought it, and the owner's, and the app makes the bot with a `store` tag (its id).
- **Its memory:** Holli Bot's AI (`convex/ai.ts`) gets `store` (the bot's id) with each request from a store bot, and `store:memoryFor` gives it the bot's pre-trained memory when the account bought it; it goes into the bot's instructions where the app left room for it (`[[holli-bot-store-memory]]`), fenced, with the rule that keeps it private. It still works for buyers after the bot is hidden from the store, and a change to it reaches every buyer's bot at once.

## Subscribers' servers

Every subscriber gets their own dedicated Vultr server running Holli Bot Computer, linked to their account, so their bots run there around the clock (`convex/servers.ts`). Like any linked Holli Bot Computer, it tells the account where it can be reached (`devices:report`), so the app on every device the subscriber signs in on becomes its remote control (`openComputer()` in `src/main.js`). It's made when they subscribe, resized when they upgrade, moved to a smaller one when they downgrade, and deleted when the subscription ends, with no manual steps. Only Stripe's webhook, the scheduler and the nightly cron make or delete servers: every function that calls Vultr is internal. The app can only ask for a failed setup to start again (`servers:retry`, at most once a minute), which schedules the same work.
...
- **Setting it up** (`convex/lib/cloudinit.ts`, cloud-init, as root at first boot): Caddy from its apt repository, Node.js 22 from nodejs.org (checked against its checksums), a desktop (XFCE on an Xvfb virtual display at 1280×800, as the `holly-display` and `holly-desktop` services, with xdotool and scrot so Holli Bot Computer can see and control it; without it the bots just have no screen), Google Chrome for the bots' browser (it opens on that desktop), and Holli Bot Computer from the site as a service (each start fetches the latest build, and every five minutes it looks for a newer one and, once no bot is working, stops so systemd starts the new one). On the server each bot gets a screen of its own, the way Grok Bot does it: Holli Bot Computer runs one wide virtual display for the bots (Xvfb, :10 or the first free one after) and gives each bot a 1280×800 part of it, where its window of the one shared Chrome (so the logins are the same for every bot) fills its screen and its mouse and keyboard act; a bot gives its screen up after half an hour unused (`computer/src/screens.mjs`). How many fit at once depends on the server's memory. Caddy serves HTTP/1.1 and HTTP/2 only (HTTP/3 would need UDP 443, which the firewall keeps closed). apt steps are tried a few times, since Ubuntu's own daily apt run can hold apt at first boot. Holli Bot Computer links itself to the subscriber's account with a one-time code (two hours), and Caddy serves it over HTTPS at `https://<ip with dashes>.sslip.io` with a Let's Encrypt certificate. Once it answers over HTTPS, the script calls `POST /servers/ready` with the user id, the one-time ready token, the address and the pairing token; on a failure it sends what went wrong instead. The log is `/var/log/holly-setup.log` on the server (Vultr's web console).
- **Ready.** A valid token makes the server `ready`; the token is then spent. The address and pairing token it reports are kept for a downgrade, when the smaller server copies the bots' files from this one. Not ready 15 minutes after it started, it's `error` with the reason (never made, no address, or Holli Bot didn't finish). The app shows the error with Try Again, which deletes what was left and starts over.
...
- **Downgrade** (`migrate`): Vultr can't make a server smaller, so a new server is made at the smaller size. While it sets up it copies the bots' files from the old one (its workspace, browser profile and plugins, through Holli Bot Computer's `/v1/export`); their bots, chats and memories are in the account already. When it reports ready it takes over and the old one is deleted. If it fails, it's deleted instead, the subscriber keeps the old one, and the reconcile tries again the next night.
- **Deleting** (`remove`): `DELETE /v2/instances/{id}`, where a 404 counts as already deleted; the server fields are cleared (`none`), and the server's Holli Bot Computer session ends. No snapshots or backups are taken: they cost extra.
...
In the [Google Cloud console](https://console.cloud.google.com/apis/credentials), create an OAuth client ID of type **Web application** (in a project whose consent screen says Holli Bot; a Macronaut client would show Macronaut's name):
...
In the [Apple Developer portal](https://developer.apple.com/account/resources/identifiers/list) you need an App ID with **Sign in with Apple**, a Services ID for the web, and a Sign in with Apple key. The [Convex Auth Apple guide](https://labs.convex.dev/auth/config/oauth/apple) walks through the screens. Holli Bot's values:
...
The deploy workflow creates `CONNECTORS_KEY` the first time (`scripts/convex-connectors-key.mjs --if-missing`). From then on, GitHub can be connected with a personal access token. Leave the key alone: a new one disconnects everyone. For the Connect buttons, set up each service's OAuth app, then add its two repository secrets. The workflow copies them to the deployment like the sign-in ones. A service without them shows as "Not set up on Holli Bot's server yet".

**Gmail** (`CONNECT_GOOGLE_ID`, `CONNECT_GOOGLE_SECRET`), in the Google Cloud project that has Holli Bot's consent screen:
...
2. API permissions → Microsoft Graph → Delegated: `offline_access`, `User.Read`, `Mail.ReadWrite` (reading, and deleting or moving email) and `Mail.Send`. Personal accounts need no admin consent. A work or school tenant may need its admin to allow Holli Bot.
...
3. Organizations that restrict OAuth apps have to approve Holli Bot before bots can reach their repositories.
...
Until this is done, everyone is on Free, and the plan page says subscriptions aren't set up yet. Start with Stripe's test mode (test cards like `4242 4242 4242 4242` pay for nothing) and `VULTR_DRY_RUN=true` (no servers are made), then switch each to the real thing.

1. **Prices.** In Stripe, Product catalog → Add product, one for each plan (Holli Bot Starter, Pro, Ultra), each with two recurring prices in USD: yearly and monthly, exactly as in the table under Subscriptions (Starter: $490 every year and $60 every month; Pro: $990 and $120; Ultra: $1,790 and $200). Put each price's id (`price_…`) in its variable: `STRIPE_PRICE_STARTER_YEARLY`, `STRIPE_PRICE_STARTER_MONTHLY`, and so on.
...
7. Subscribe from the app with a test card. It should go straight to setting up the computer, then open the app, and the Bot Computer settings should show the computer.
...
Bots run on Holli Bot's DeepSeek key and the account's AI credits (above). In the [DeepSeek platform](https://platform.deepseek.com/api_keys), make an API key for Holli Bot and top up the balance, then set it as the repository secret **`DEEPSEEK_API_KEY`** (or in the Convex dashboard) and run Deploy Convex. Until it's set, the app says Holli Bot's AI isn't set up yet, and bots only work for accounts that saved their own DeepSeek key before credits.

Free's daily budget, what all Free accounts together may spend on it a day, is **`FREE_DAILY_BUDGET_USD`** in the Convex dashboard (Settings → Environment Variables), in US dollars: `5` when it isn't set, and `0` turns Free's AI off. It takes effect at once, with no deploy.

### 7. Check

Open https://xgamer791.github.io/holly-bot/ and tap Sign In. A button that isn't ready says so ("Apple sign-in isn't set up yet"). The app needs an account, so there's no way past the sign-in screen until one works. A new account opens on Free; the paid plans work once Stripe and Vultr are set up (step 5).
...
On the Holli Bot site and on `http://localhost` (Holli Bot Computer's own page on the computer, `npm run serve`): the only places Google and Apple can send you back to.

- Holli Bot Computer's QR code for a tunnel or `--public-url` address is a link to the site with the pairing in the `#connect=` fragment, so the phone signs in there and runs the current build. An older link to the tunnel address itself hands over to the site the same way (`handOverToSite` in `src/main.js`).
