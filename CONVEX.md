...
- `convex/ai.ts` and `convex/credits.ts`: Holly Bot's AI, GLM 5.3 Flash and DeepSeek V4 Pro through OpenRouter on Holly Bot's key, at `https://impressive-ferret-800.convex.site/ai/chat/completions`, and each account's monthly AI credits for it (below), with `convex/lib/credits.ts` (the models and their prices, what a request costs, the months of credits).
...
Bots think with GLM 5.3 Flash (Z.ai's) or DeepSeek V4 Pro through OpenRouter, on Holly Bot's own key, `OPENROUTER_API_KEY`, which only the deployment holds. Each plan gives the account AI credits every month (`credits` in `convex/lib/plans.ts`: Starter $10, Pro $20, Ultra $35 of what the AI costs, which the app shows as 1,000, 2,000 and 3,500 credits). There are no API keys for AI in the app any more.

- **Requests.** The app and Holly Computer send each bot request, OpenAI-style, to `POST /ai/chat/completions` (`convex/ai.ts`) with the account's session JWT as the bearer token (`sessionToken` on `src/account/cloud-db.js`, used by `src/core/providers/index.js`). The route checks the session is still open and the subscription lets the account in (as `requireSubscriber` does), takes only DeepSeek-style chat parameters (the form the app has always sent) and the models `glm-flash` and `deepseek-v4-pro` (`deepseek-flash`, Flash's name before 1.25.0, runs as `glm-flash`), caps the output at 65,536 tokens, and passes it to OpenRouter as `z-ai/glm-5.3-flash` or `deepseek/deepseek-v4-pro-0813` (`AI` in `convex/lib/credits.ts`), with `thinking` and `reasoning_effort` as OpenRouter's `reasoning` (high effort unless the app says). GLM 5.3 Flash always thinks: asked not to (background jobs do), it thinks at low effort and gets 8,192 more output tokens for it. Answers go back to the app as they come, in DeepSeek's form (OpenRouter's `reasoning` as `reasoning_content`), so the app and Holly Computer read them as they always have. Nothing of a request or answer is kept.
...
- **Metering.** OpenRouter's last event says what the request cost (`usage.cost`, in dollars) and the tokens it used, and `credits:charge` takes exactly that cost off the account's balance, in millionths of a dollar. A request the app cut off before that last event is charged an estimate at the model's usual OpenRouter price (`prices` in `AI`): its input split the way the account's own was cached that month, and what was streamed.
...
- **The app** reads `credits:mine` for Settings → Usage: a bar of what's left, the credits left and the refill date. It also says `ready` (whether `OPENROUTER_API_KEY` is set); until it is, bots keep using a DeepSeek key an account had saved before credits, and accounts without one can't run bots.
- **OpenRouter's credits** pay for everyone. When OpenRouter turns Holly Bot's key down (out of credits, or a wrong key), members see "Holly Bot's AI is unavailable right now", and the deployment's logs say `OpenRouter refused Holly Bot's key` with the reason. Keep the OpenRouter account topped up (auto top-up on its Credits page does it): at most, a month costs the sum of every subscriber's allowance.
...
- copies the connector apps' secrets, the Stripe and Vultr variables, and the OpenRouter key, the same way (steps 4 to 6),
...
Bots run on Holly Bot's OpenRouter key and the account's AI credits (above). In [OpenRouter](https://openrouter.ai/settings/keys), make an API key for Holly Bot and buy credits (turn on auto top-up so they don't run out), then set it as **`OPENROUTER_API_KEY`** in the Convex dashboard (Settings → Environment Variables), which takes effect at once, or as a repository secret and run Deploy Convex. `DEEPSEEK_API_KEY`, which Holly Bot used before OpenRouter, isn't used any more and can be deleted. Until `OPENROUTER_API_KEY` is set, the app says Holly Bot's AI isn't set up yet, and bots only work for accounts that saved their own DeepSeek key before credits.
