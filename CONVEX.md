...
- `convex/ai.ts` and `convex/credits.ts`: Holly Bot's AI, DeepSeek V4.1 Flash and V4 Pro through OpenRouter on Holly Bot's key, at `https://impressive-ferret-800.convex.site/ai/chat/completions`, and each account's monthly AI credits for it (below), with `convex/lib/credits.ts` (the models and their prices, what a request costs, the months of credits).
...
Bots think with DeepSeek V4.1 Flash or V4 Pro through OpenRouter, on Holly Bot's own key, `OPENROUTER_API_KEY`, which only the deployment holds. Each plan gives the account AI credits every month (`credits` in `convex/lib/plans.ts`: Starter $10, Pro $20, Ultra $35 of what the AI costs, which the app shows as 1,000, 2,000 and 3,500 credits). There are no API keys for AI in the app any more.

- **Requests.** The app and Holly Computer send each bot request, OpenAI-style, to `POST /ai/chat/completions` (`convex/ai.ts`) with the account's session JWT as the bearer token (`sessionToken` on `src/account/cloud-db.js`, used by `src/core/providers/index.js`). The route checks the session is still open and the subscription lets the account in (as `requireSubscriber` does), takes only DeepSeek-style chat parameters (the form the app has always sent) and the models `deepseek-flash` and `deepseek-v4-pro` (`glm-flash`, Flash's name in 1.25.0 when it ran GLM 5.3 Flash, runs as `deepseek-flash`), caps the output at 65,536 tokens, and passes it to OpenRouter as `deepseek/deepseek-v4.1-flash` or `deepseek/deepseek-v4-pro-0813` (`AI` in `convex/lib/credits.ts`: the releases DeepSeek's own API serves), with `thinking` and `reasoning_effort` as OpenRouter's `reasoning`. Answers go back to the app as they come, in DeepSeek's form (OpenRouter's `reasoning` as `reasoning_content`), so the app and Holly Computer read them as they always have. Nothing of a request or answer is kept.
...
- **Metering.** OpenRouter's last event says what the request cost (`usage.cost`, in dollars) and the tokens it used, and `credits:charge` takes exactly that cost off the account's balance, in millionths of a dollar. A request the app cut off before that last event is charged an estimate at the model's list price (`prices` in `AI`, DeepSeek's): its input split the way the account's own was cached that month, and what was streamed.
