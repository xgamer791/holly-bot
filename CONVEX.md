...
- `convex/ai.ts` and `convex/credits.ts`: Holly Bot's AI, DeepSeek's models through OpenRouter on Holly Bot's key, at `https://impressive-ferret-800.convex.site/ai/chat/completions`, and each account's monthly AI credits for it (below), with `convex/lib/credits.ts` (the models, DeepSeek's prices, what a request costs, the months of credits).
...
Bots think with DeepSeek's models through OpenRouter, on Holly Bot's own key, `OPENROUTER_API_KEY`, which only the deployment holds. (Without it, the server calls DeepSeek's own API with `DEEPSEEK_API_KEY`, as it did before OpenRouter.) Each plan gives the account AI credits every month (`credits` in `convex/lib/plans.ts`: Starter $10, Pro $20, Ultra $35 of what the AI costs, which the app shows as 1,000, 2,000 and 3,500 credits). There are no API keys for AI in the app any more.

- **Requests.** The app and Holly Computer send each bot request, OpenAI-style, to `POST /ai/chat/completions` (`convex/ai.ts`) with the account's session JWT as the bearer token (`sessionToken` on `src/account/cloud-db.js`, used by `src/core/providers/index.js`). The route checks the session is still open and the subscription lets the account in (as `requireSubscriber` does), takes only DeepSeek's chat parameters and the models `deepseek-flash` and `deepseek-v4-pro`, caps the output at 65,536 tokens, and passes it to OpenRouter as `deepseek/deepseek-v4.1-flash` or `deepseek/deepseek-v4-pro-0813` (`OPENROUTER` in `convex/lib/credits.ts`: the releases DeepSeek's own API serves), with DeepSeek's `thinking` and `reasoning_effort` as OpenRouter's `reasoning`. Answers go back to the app as they come, in DeepSeek's form (OpenRouter's `reasoning` as `reasoning_content`, and the app's names for the models), so the app and Holly Computer work the same on either. Nothing of a request or answer is kept.
- **Providers.** Requests ask OpenRouter only for providers that don't keep or train on what they're sent (`provider: { data_collection: "deny" }`), which leaves out DeepSeek's own API (OpenRouter lists it as one that may train on prompts) and keeps the privacy policy's promises, Gmail's included. OpenRouter picks among the rest by price and uptime, and, for requests with tools, by how reliably each calls them. Each account's requests carry a `session_id` (its id, hashed), so they keep to one provider and reuse its cache of their input.
- **Metering.** OpenRouter's last event says what the request cost (`usage.cost`, in dollars) and the tokens it used, and `credits:charge` takes exactly that cost off the account's balance, in millionths of a dollar. On DeepSeek's own API, the last event has only the tokens (from its cache, new input, output), and `charge` prices them at `PRICES` in `convex/lib/credits.ts` (DeepSeek's list prices, half price outside its peak hours); keep `PRICES` in step with DeepSeek's pricing page. A request the app cut off before that last event is charged an estimate at DeepSeek's list prices (with no off-peak discount on OpenRouter): its input split the way the account's own was cached that month, and what was streamed.
- **Holds.** Before a request goes out, `credits:admit` holds back the most it could cost (all its input new, all the output it may ask for), or what's left when that's less, and `charge` gives the hold back as it charges. So requests at once can't spend more than is left, and an account never goes more than one request's worth below zero.
...
- **The app** reads `credits:mine` for Settings → Usage: a bar of what's left, the credits left and the refill date. It also says `ready` (whether `OPENROUTER_API_KEY` or `DEEPSEEK_API_KEY` is set); until one is, bots keep using a DeepSeek key an account had saved before credits, and accounts without one can't run bots.
- **OpenRouter's credits** pay for everyone. When OpenRouter turns Holly Bot's key down (out of credits, or a wrong key), members see "Holly Bot's AI is unavailable right now", and the deployment's logs say `OpenRouter refused Holly Bot's key` with the reason. Keep the OpenRouter account topped up (auto top-up on its Credits page does it): at most, a month costs the sum of every subscriber's allowance. On DeepSeek's own API, it's the DeepSeek balance, and the logs say `DeepSeek refused Holly Bot's key`.
...
- copies the connector apps' secrets, the Stripe and Vultr variables, and the AI keys (OpenRouter's and DeepSeek's), the same way (steps 4 to 6),
...
Bots run on Holly Bot's OpenRouter key and the account's AI credits (above). In [OpenRouter](https://openrouter.ai/settings/keys), make an API key for Holly Bot and buy credits (turn on auto top-up so they don't run out), then set it as **`OPENROUTER_API_KEY`** in the Convex dashboard (Settings → Environment Variables), which takes effect at once, or as a repository secret and run Deploy Convex. A key from the [DeepSeek platform](https://platform.deepseek.com/api_keys) as **`DEEPSEEK_API_KEY`** also works, and is used only while `OPENROUTER_API_KEY` isn't set; once OpenRouter works, it can go. Until one of them is set, the app says Holly Bot's AI isn't set up yet, and bots only work for accounts that saved their own DeepSeek key before credits.
