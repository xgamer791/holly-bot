// Holly Computer's tunnel (computer/src/tunnel.mjs): an address goes to the
...
test('the address is told once cloudflared has connected and it answers as this computer', async () => {
  ...
  // cloudflared reaches Holly Computer on 127.0.0.1, not "localhost" (which can mean ::1).
...
test('tunnelOrigin: 127.0.0.1 unless Holly Computer listens on one address only', () => {
