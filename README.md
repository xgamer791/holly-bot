...
- Bots think with **Holly Bot's AI**, which Holly Bot's server runs through OpenRouter with its own key (`convex/ai.ts`). Nobody brings a key. Each bot runs **GLM 5.3 Flash** from Z.ai (`glm-flash`, the default: smart, fast, a 1M-token context, it sees images, and it always thinks) or **DeepSeek V4 Pro** (`deepseek-v4-pro`: deeper thinking, several times the credits), picked in the bot's profile. Flash was DeepSeek V4.1 Flash until 1.25.0; bots that had it now run GLM 5.3 Flash. OpenRouter only hands requests to providers that don't keep or train on them, so DeepSeek's own servers (in China) aren't used.
