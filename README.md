...
- **One icon set, Lucide** (lucide.dev, lucide-static 1.48.0), drawn as Lucide draws them, for every icon Holli Bot shows, Holli Bot for Windows' settings window included; the only marks that aren't Lucide's are Apple's, Google's and GitHub's own. Every close button is the same round button, in the surface's color with a hairline around it and Lucide's x (smaller on a card, a notice or an attachment), and back buttons match it. Every switch is the iOS size Settings uses. In Light, boxes, plain buttons, chips and tabs are white with a hairline, so they show on a sheet's gray, and the part of a segmented control that's on is white, like Monthly | Yearly on the plan page.
