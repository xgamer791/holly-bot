...
const COMPUTER_STATES = {
  ...
  blocked: "on, but that computer's network blocks the secure tunnel Holly Bot Computer uses (Cloudflare Tunnel, outbound port 7844), so Holly Bot can't reach it: the user should allow that port on that network, use another network, or start Holly Bot Computer with --public-url (in the Holly Bot Computer app on Windows: Your own address, in its Settings)",
  hidden: "on, but Holly Bot Computer runs there without --tunnel, so Holly Bot can't reach it: the user should restart it with --tunnel (in the Holly Bot Computer app on Windows: turn on Secure tunnel in its Settings)",
  off: "off: Holly Bot Computer isn't running there. Once the user starts it (the Holly Bot Computer app on Windows, or node holly-computer.mjs), Holly Bot connects to it (the first time, the app asks them to tap Connect)",
  old: 'its Holly Bot Computer is out of date: the user should restart it (the Holly Bot Computer app on Windows updates itself as it starts), or download holly-computer.mjs again and start it',
};
...
export function buildSystemPrompt({ app, agent, thread, tools }) {
  ...
  lines.push('', '## Your computers',
    ...
    ...(otherPcs.length
      ...
      : herePc ? [] : ['No computer of theirs is linked to Holly Bot yet: they can run Holly Bot Computer on their PC or Mac (on Windows, the Holly Bot Computer app installs it) and sign in on the page it opens (the computer button at the top right of the app shows how).']),
