...
export const memoryTools = [
  {
    ...
    async run(args, ctx) {
      const owner = args.shared ? SHARED_ID : ctx.agent.id;
      ...
      return {
        ...
        display: { kind: 'memory', op: action === 'merged' ? 'update' : 'add', text: memory.text, shared: !!args.shared },
      };
    },
  },
  {
    ...
    async run(args, ctx) {
      const res = await ctx.app.memory.search(ctx.agent.id, args.query, { limit: args.limit || 10, includeShared: true, minScore: 0.05 });
      return {
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      if (!m || (m.agentId !== ctx.agent.id && m.agentId !== SHARED_ID)) return { content: `No memory ${args.id} found.`, isError: true };
      const next = await ctx.app.memory.update(args.id, { text: args.text, ...(args.importance ? { importance: args.importance } : {}) });
      return { content: `Updated ${next.id}.`, display: { kind: 'memory', op: 'update', text: next.text } };
    },
  },
  {
    name: 'forget',
    ...
    async run(args, ctx) {
      ...
      return { content: `Deleted ${args.id}.`, display: { kind: 'memory', op: 'delete', text: m.text } };
    },
  },
  {
    name: 'core_memory',
